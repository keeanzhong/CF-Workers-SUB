/**
 * CF-Workers-SUB 旗舰管理版 (修复增强版)
 * 修复内容：解决潜在的格式问题，增加JSON解析容错，移除不可见字符
 */
...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            // 1. 检查 KV 绑定状态 (这是最常见的配置错误)
            if (!env.KV && url.pathname === '/admin_panel') {
                return new Response(`配置错误：未找到 KV 绑定。<br>请在 Cloudflare 后台 Settings -> Variables 绑定 KV Namespace，变量名必须为 <b>KV</b>`, { headers: { 'Content-Type': 'text/html;charset=utf-8' } });
            }
...
async function handleAdminPanel(env) {
    ...
    // 1. 获取日志并排序 (增加了 JSON.parse 的容错处理)
